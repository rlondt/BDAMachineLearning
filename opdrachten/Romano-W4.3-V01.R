...
##############################################################
#inlezen bestand
df_read <- read.csv("datafiles/wisc_bc_data.csv")
...
# The false positive ratio = FP/(TN+FP) = 4/(4+85)
FprFromCM <- function(cm){
  cm$table[1,2]/(cm$table[2,2]+cm$table[1,2])
}

# The false negative ratio = FN
FnrFromCM <- function(cm){
  cm$table[2,1]/(cm$table[2,1]+cm$table[1,1])
}
